...
fields.forEach(field => {
  ...
  $input.addEventListener('keyup', debounce(async (e) => { // 키보드 입력 시마다 유효성 검증
    let isValid;
    if (field.id === 'passwordCheck') { // 비밀번호 확인 필드인 경우
      const passwordValue = document.getElementById('password').value; // 비밀번호 필드의 값 가져오기
      isValid = await field.validator($input.value, passwordValue); // 유효성 검증 함수 호출
    } else {
      isValid = await field.validator($input.value); // 유효성 검증 함수 호출
    }
